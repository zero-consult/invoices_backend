package org.zero_consult.payslip_backend.cron;
...
import org.zero_consult.payslip_backend.services.PayslipService;
import org.zero_consult.payslip_backend.services.TimesheetApiService;
...
public class PayslipPdfGenerator {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<org.zero_consult.payslip_backend.entities.Allowance> allowances = new ArrayList<>();
        org.zero_consult.payslip_backend.entities.Allowance allowance = new org.zero_consult.payslip_backend.entities.Allowance();
    ...
    private static void generatePayslipPdf(Payslip payslip, String payslipFile, Employee employee, List<TimesheetEntry> timesheetEntries) {
        try (InputStream payslipReportStream = PayslipPdfGenerator.class.getClassLoader().getResourceAsStream("jrxml/payslip.jrxml");
            ...
            double grossWage = ((workHours + holidayHours + sickHours) / 8d) * payslip.getGrossSalary() / 20;
            if (workHours > 0) {
                allowances.add(new Allowance("" + workHours / 8, formatCurrency(payslip.getGrossSalary() / 20), "", formatCurrency((workHours / 8d) * payslip.getGrossSalary() / 20), ""));
            }
            if (holidayHours > 0) {
                allowances.add(new Allowance("" + holidayHours / 8, formatCurrency(payslip.getGrossSalary() / 20), "", formatCurrency((holidayHours / 8d) * payslip.getGrossSalary() / 20), ""));
            }
            if (sickHours > 0) {
                allowances.add(new Allowance("" + sickHours / 8, formatCurrency(payslip.getGrossSalary() / 20), "", formatCurrency((sickHours / 8d) * payslip.getGrossSalary() / 20), ""));
            }
